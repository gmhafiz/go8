...
type bookR struct {
	Authors AuthorSlice `db:"Authors" boil:"Authors" json:"Authors" toml:"Authors" yaml:"Authors"`
}
...
var (
	bookAllColumns            = []string{"id", "title", "published_date", "image_url", "description", "created_at", "updated_at", "deleted_at"}
	bookColumnsWithoutDefault = []string{"title", "published_date", "image_url", "description", "updated_at", "deleted_at"}
	bookColumnsWithDefault    = []string{"id", "created_at"}
...
func (o *Book) Authors(mods ...qm.QueryMod) authorQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"book_authors\" on \"authors\".\"id\" = \"book_authors\".\"author_id\""),
		qm.Where("\"book_authors\".\"book_id\"=?", o.ID),
	)
...
func (bookL) LoadAuthors(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBook interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"authors\".id, \"authors\".first_name, \"authors\".middle_name, \"authors\".last_name, \"authors\".created_at, \"authors\".updated_at, \"authors\".deleted_at, \"a\".\"book_id\""),
		qm.From("\"authors\""),
		qm.InnerJoin("\"book_authors\" as \"a\" on \"authors\".\"id\" = \"a\".\"author_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.FirstName, &one.MiddleName, &one.LastName, &one.CreatedAt, &one.UpdatedAt, &one.DeletedAt, &localJoinCol)
		if err != nil {
...
func (o *Book) RemoveAuthors(ctx context.Context, exec boil.ContextExecutor, related ...*Author) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"book_authors\" where \"book_id\" = $1 and \"author_id\" in (%s)",
