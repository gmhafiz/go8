...
type authorR struct {
	Books BookSlice `db:"Books" boil:"Books" json:"Books" toml:"Books" yaml:"Books"`
}
...
var (
	authorAllColumns            = []string{"id", "first_name", "middle_name", "last_name", "created_at", "updated_at", "deleted_at"}
	authorColumnsWithoutDefault = []string{"first_name", "middle_name", "last_name", "created_at", "updated_at", "deleted_at"}
...
func (o *Author) Books(mods ...qm.QueryMod) bookQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"book_authors\" on \"books\".\"id\" = \"book_authors\".\"book_id\""),
		qm.Where("\"book_authors\".\"author_id\"=?", o.ID),
	)
...
func (authorL) LoadBooks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuthor interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"books\".id, \"books\".title, \"books\".published_date, \"books\".image_url, \"books\".description, \"books\".created_at, \"books\".updated_at, \"books\".deleted_at, \"a\".\"author_id\""),
		qm.From("\"books\""),
		qm.InnerJoin("\"book_authors\" as \"a\" on \"books\".\"id\" = \"a\".\"book_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Title, &one.PublishedDate, &one.ImageURL, &one.Description, &one.CreatedAt, &one.UpdatedAt, &one.DeletedAt, &localJoinCol)
		if err != nil {
...
func (o *Author) AddBooks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Book) error {
	...
	for _, rel := range related {
		query := "insert into \"book_authors\" (\"author_id\", \"book_id\") values ($1, $2)"
...
func (o *Author) RemoveBooks(ctx context.Context, exec boil.ContextExecutor, related ...*Book) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"book_authors\" where \"author_id\" = $1 and \"book_id\" in (%s)",
...
func FindAuthor(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Author, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"authors\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Author) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"authors\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func AuthorExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"authors\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
