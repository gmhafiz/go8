...
var BookAuthorWhere = struct {
	BooksID  whereHelperint64
	AuthorID whereHelperint64
}{
	BooksID:  whereHelperint64{field: "\"book_authors\".\"books_id\""},
...
var (
	bookAuthorAllColumns            = []string{"books_id", "author_id"}
	bookAuthorColumnsWithoutDefault = []string{}
	bookAuthorColumnsWithDefault    = []string{"books_id", "author_id"}
	bookAuthorPrimaryKeyColumns     = []string{"books_id", "author_id"}
)
...
func (bookAuthorL) LoadAuthor(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBookAuthor interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`authors`),
		qm.WhereIn(`authors.author_id in ?`, args...),
...
func (o *BookAuthor) SetAuthor(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Author) error {
	...
	values := []interface{}{related.AuthorID, o.BooksID, o.AuthorID}
...
func FindBookAuthor(ctx context.Context, exec boil.ContextExecutor, booksID int64, authorID int64, selectCols ...string) (*BookAuthor, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"book_authors\" where \"books_id\"=$1 AND \"author_id\"=$2", sel,
	)

	q := queries.Raw(query, booksID, authorID)
...
func (o *BookAuthor) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"book_authors\" WHERE \"books_id\"=$1 AND \"author_id\"=$2"
...
func (o *BookAuthor) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindBookAuthor(ctx, exec, o.BooksID, o.AuthorID)
	if err != nil {
		return err
	}
...
func BookAuthorExists(ctx context.Context, exec boil.ContextExecutor, booksID int64, authorID int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"book_authors\" where \"books_id\"=$1 AND \"author_id\"=$2 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, booksID, authorID)
	}
	row := exec.QueryRowContext(ctx, sql, booksID, authorID)
