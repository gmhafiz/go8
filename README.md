...
A. Have both a postgres database, redis(optional) instance, and elasticsearch ready.

If not, you can run the following command if you have `docker-compose` installed:
 
    docker-compose -f deployments/docker-compose.yml up -d postgres redis elasticsearch


For Elasticsearch, you may need to increase `max_map_count` either temporarily by `sudo sysctl -w vm.max_map_count=262144` or permanently by editing `/etc/sysctl`
 
    vim /etc/sysctl.conf
    vm.max_map_count=262144

Then either reboot or run `sudo sysctl -w vm.max_map_count=262144`
